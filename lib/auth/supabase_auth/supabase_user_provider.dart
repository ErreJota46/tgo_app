...
class TGoSupabaseUser extends BaseAuthUser {
  TGoSupabaseUser(this.user);
  User? user;
  @override
...
Stream<BaseAuthUser> tGoSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = TGoSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
